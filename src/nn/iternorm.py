"""
This code was extracted from https://github.com/zhiCHEN96/ConceptWhitening/tree/final_version, official
implementation of the paper Concept Whitening for Interpretable Image Recognition (Nature Machine Intelligence 2, 772–782 (2020)). 

The authors used the following reference:
...
class iterative_normalization_py(torch.autograd.Function):
    ...
    def forward(ctx, *args, **kwargs):
        ...
        if training:
            ...
            #Sigma = torch.baddbmm(eps, P[0], 1. / m, xc, xc.transpose(1, 2)) # DEPRECATED
            ...
            for k in range(ctx.T):
                #P[k + 1] = torch.baddbmm(1.5, P[k], -0.5, torch.matrix_power(P[k], 3), Sigma_N) # DEPRECATED
    ...
    def backward(ctx, *grad_outputs):
        ...
        for k in range(ctx.T, 1, -1):
            ...
            g_tmp = g_P.matmul(sn)
            #g_P.baddbmm_(1.5, -0.5, g_tmp, P2) # DEPRECATED
            g_P.baddbmm_(g_tmp, P2, beta=1.5, alpha=-0.5)
            #g_P.baddbmm_(1, -0.5, P2, g_tmp) # DEPRECATED
            g_P.baddbmm(P2, g_tmp, beta=1, alpha=-0.5)
            #g_P.baddbmm_(1, -0.5, P[k - 1].matmul(g_tmp), P[k - 1]) #DEPRECATED
